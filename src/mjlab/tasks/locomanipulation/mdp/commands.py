...
class MotionCommand(CommandTerm):
  ...
  def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    # Optional interactive object in the scene (e.g., largebox).
    ...
    self._object_model: mujoco.MjModel | None = None
    self._object_color = np.array(cfg.viz.object_color, dtype=np.float32)
  ...
  def _debug_vis_impl(self, visualizer: DebugVisualizer) -> None:
    ...
    if self.cfg.viz.mode == "ghost":
      ...
      # If an object entity exists, also set its free joint from the motion so
      # that the ghost box aligns with the commanded trajectory.
      if self.object is not None:
...
class MotionCommandCfg(CommandTermCfg):
  ...
  class VizCfg:
    ...
    object_color: tuple[float, float, float, float] = (0.5, 0.5, 0.7, 0.5)
